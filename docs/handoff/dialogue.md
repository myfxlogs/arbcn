...
- **部署闭环（业主「直接执行吧」+ 授权 re-arm）**：`ARBCN_SIM_EXEC_VENUE=okx_demo` 写入 /etc/arbcn/arbcn-monitor.env → inode 换名法换新二进制（cp .new → mv，避 Text file busy）→ systemctl restart（sudo）→ journal 确认 `sim mirror executor enabled venue="okx_demo"` + `migrations applied count=1`（0012 sim_order_executions）+ healthz ok + 50052 正常。
- **确认路径端到端实测（业主授权 re-arm 后）**：DELETE trigger_states rule_id=7 → funding_drill 演练单生成（venue=okx/binance，funding_hedge，qty 20000，ref ~63480）→ `POST /arbcn.sim.v1.SimService/ConfirmSimOrder`（Connect JSON + Connect-Protocol-Version: 1）→ **门禁 fail-closed 拒单**：`SPREAD_DRIFT: 预期年化 7.90% → 6.24%（回落 21.01%，超阈值 ±20%）` 等。**8 次有界重试全拒**（13:50–14:06，每 5 分钟演练单生成一次）。根因 = **市场窗口未开非代码 bug**：drill 规则 cond 用 avg_30d 触发（avg_30d>5 && <15），确认门禁用 **current 单点 funding** 比对——当前全实体 current vs avg_30d 分歧 >20%（okx BTC 7.90→6.15 回落 22%、binance BTC 5.77→9.70 上行 68%、okx ETH −37%、binance ETH 大正幅，跨所反向分歧）。按 D-061② 环境无窗口 = 正确输出非失败。
- **遗留（待市场窗口）**：真实 OKX demo 成交回读尚未落地。待 funding 回到 30d 均值 ±20% 内（约 okx BTC ≥6.3% 或 binance BTC ≤6.9%）→ re-arm（需业主再授权，重试一次性授权已用完）→ 确认 → 查 sim_order_executions 落 exchange_order_id + fill_price → 施工表 🔄→✅。drill 现静止 unarmed。
