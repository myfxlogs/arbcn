...
- 施工表:
  ...
  | M3：模拟执行验证（b 机制收敛 + §5.3 历史统计前置 → c 一键确认 UI） | ⬜ | D-034/D-036 · 下一项 |
- 阻塞/待决策:
  ...
  - **M3-a 已验收（D-035）**；M3-b 细化设计待排（testnet key 豁免条款 D-034：隔离+SIMULATED 标记+只读，真金零密钥不变；M3 文档审计 D-036：收敛口径修正 + G1–G5 已落 spec，§3.1.2 驱动与 §5.3 历史回填须含进 M3-b 细化设计）。
...
- 下一步: **M3-b 细化设计**（04-m3-spec.md：§3.1.2 运行驱动接线方案（G1，OnActive 扩展）+ §5.3 历史收敛分析前置任务（数据回填 + 统计报告）+ 白名单显式配置，M3-a 复审 M2 接受项）。
- 清扫上翻: 本次 review 教训入 practices.md #6-#10（刻度统一/NaN 门禁/状态+从属行原子/信任边界标注/时钟注入覆盖全路径）；追溯深审 + M3-a 复审结论入 decisions.md D-035；M3 文档审计结论入 D-036（收敛口径修正 + G1–G5）；对话落 dialogue.md #39/#40/#41。
