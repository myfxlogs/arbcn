...
- 施工表:
  ...
  | 业主定混合比例（60/40 或 75/25） | ⬜ | 待业主 |
  | 出入金通道验证（1 万小额 OTC） | ⬜ | 业主执行 |
  | **监控 v1 技术规格 → 施工 agent** | 🔄 | 下一项 |
  | 台账/归因 v2 | ⬜ | 监控 v1 后 |
- 阻塞/待决策:
  - **混合比例**：60/40（推荐，名义 ~3.1%）或 75/25（收益最大化，敞口 60–75%）。业主拍板后落 D-024 并定稿。
- 下一步: ① 监控 v1 技术规格（数据源/指标/阈值/告警/RMB 折算）→ 施工；② 业主执行出入金验证。
- 清扫上翻: 本轮对话已落 dialogue.md（#11/#12）；决策已落 decisions.md（D-020~D-023）；对比表已落 docs/design/01-plan.md §13。
