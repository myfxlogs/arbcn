...
- 现状: **M1 验收通过**——监控系统全链路建成（30 分钟实测：1,038 行 facts / 状态机全路径 / 仪表盘可访问）。档位已定稿 50/50（D-026），40/60 变体业主可随时切。挂起项：SMTP 真实投递待业主授权码、systemd 安装待业主。
...
- 下一步: M1-i 补丁收口 → 业主部署两动作（SMTP 授权码 + systemd 安装）→ M2 规格。
