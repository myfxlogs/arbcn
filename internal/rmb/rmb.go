...
//
// 口径（D-023 例：稳定币 4.5–6% 折算人民币后，升值 3% 情景净 1.5–3%）：
//
...
type Converted struct {
	Fact        fact.Fact
	RMBValue    float64 // RMB 净收益视角；覆盖 kind 且汇率可用 = Value − 年化升值；否则 = Value
...
//
//	raw = (last/first − 1) / 天数 × 365   （正 = USDCNH 走强 = 美元升值）
//	return = −raw                          （正 = 人民币升值）
//
...
func AnnualizedRMBAppreciation(series []fact.Fact) float64 {
	...
	raw := (last.Value/first.Value - 1) / days * 365
	return -raw
}

// Convert 对 facts 做 RMB 折算：
//   - 覆盖 kind（funding/defi_rate/deposit_rate）且 fx 非 nil →
//     RMBValue = Value − appreciation（USD 收益率 − 年化人民币升值），FXRate/FXAvailable 回填；
