...
//
//	go test -tags=live -run TestLivePoll -v ./internal/collect/optionsiv/
//
// 中国大陆直连 Deribit 可能被墙；失败时错误即证据（含端点 URL），
// 由决策层裁决换源（§5 列名 OKX opt-summary 为备选）/代理/降级人工录入，施工方不自行决策。
package optionsiv
...
// TestLivePoll：对真实 Deribit DVOL 指数拉取一轮 BTC/ETH。
